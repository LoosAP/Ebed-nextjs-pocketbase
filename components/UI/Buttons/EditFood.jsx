  ...
  const handleSubmit = async (event) => {
    ...
    try {
      ...
      if (querySnapshot.items.length > 0) {
        ...
        const result = await pb.collection("food").update(food.id, data);
  ...
  return (
    ...
    >
      <div className="fixed p-6 transform -translate-x-1/2 -translate-y-1/2 bg-white rounded-md shadow-lg top-1/2 left-1/2">
        <h2 className="mb-4 text-lg font-medium">Étel szerkesztése</h2>
        <button
          type="button"
...
const EditFood = ({ className, food, onRefresh, status, setStatus }) => {
